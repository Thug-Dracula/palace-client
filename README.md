...
> what OpenPalace writes; Taj writes `"PC4125"` and the captured PalaceChat 5
> client wrote `"PC5027"`. `palace_walker.py` overwrites it with `"SCOUT1"`, and
...
`~/palace-corpus/TAURI-CLIENT-SCOPE.md` (`palace-room` depends on `palace-wire`) and
...
Validated against `~/palace-corpus/payloads_all/` (799 real payloads harvested from a
live PC5 server):
