...
The captures settle it. In `~/palace-corpus/pcap_extract/animanic_pass2_dump.txt`,
...
`~/palace-corpus/tools/palace_walker.py::_store_asset` — the known-good Python
parser — from `~/palace-corpus/props_from_capture/`; the packet lengths came from
`~/palace-corpus/pcap_extract/animanic_pass2_dump.txt` and `…pass3_dump.txt`.
...
(`PalaceProtocolRef.txt`) and `~/palace-corpus/tools/palace_walker.py`. The captured
