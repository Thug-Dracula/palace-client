...
cargo run -p palace-prop --release --bin prop-tool -- \
    inventory ~/palace-corpus/pserver.prp ~/palace-corpus/props_harvested

# Differential against the Python oracles (about 7 minutes):
python3 tools/diff_corpus.py ~/palace-corpus/reference/PalacePalette.as \
...
2. **`~/palace-corpus/tools/prop_decoder.py`** — the pre-existing, independently
...
`~/palace-corpus/sprites_anim/` contains 44×44 RGBA PNGs of props, produced by a
...
```bash
cd ~/ProgramFiles/palace-client-prop

cargo test -p palace-prop

cargo run -p palace-prop --release --bin prop-tool -- \
    inventory ~/palace-corpus/pserver.prp \
              ~/palace-corpus/props_harvested ~/palace-corpus/props_all \
              ~/palace-corpus/props_from_live ~/palace-corpus/props_recovered2 \
              ~/palace-corpus/props_recovered3 ~/palace-corpus/props_recovered4 \
              ~/palace-corpus/props_recovered6 ~/palace-corpus/props_from_capture \
              ~/palace-corpus/props_harvest_final ~/palace-corpus/props_test

PALACE_PROP_CORPUS="$HOME/colosseum/pserver.prp:$HOME/colosseum/props_harvested" \
...
python3 tools/gen_palette.py ~/palace-corpus/reference/PalacePalette.as \
    --check crates/palace-prop/src/palette.rs

python3 tools/diff_prerendered.py ~/palace-corpus/reference/PalacePalette.as \
    ~/palace-corpus/sprites_anim ~/palace-corpus/props_harvested ~/palace-corpus/props_all \
...
| Source | Used for |
|---|---|
| `~/palace-corpus/reference/repos/OpenPalace/PalaceClient/src/net/codecomposer/palace/model/PalaceProp.as` | **Primary.** All five decoders and the S20 encoder. |
| `…/PalacePalette.as` | `clutARGB`, the 8-bit palette |
| `…/PalacePropFormat.as` | the format-name enum |
| `~/palace-corpus/reference/repos/Taj/Palace/PalaceProp.cs` + `Messages/Flags/PropFormatFlags.cs` | Independent C# port; source of the dither/loop-count divergences in §4 |
| `~/palace-corpus/tools/prop_decoder.py` | Independent 8-bit differential oracle (read-only) |
| `~/palace-corpus/PRP-FORMAT.md` | The `.prp` roster container, CRC seed and slice rules |
| `~/palace-corpus/reference/repos/QPalace/`, `~/palace-corpus/reference/repos/pserver/` | Checked for a competing prop decoder; neither has one (QPalace's matches are IPC-unrelated, pserver only handles the container) |
| `~/palace-corpus/reference/prp-variants/` | Six roster variants used to widen the 32-bit/20-bit/S20 sample and to characterise the corrupt file |
