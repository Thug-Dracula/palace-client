...
//! ```
//!
//! Everything defaults to the local corpus under `~/palace-corpus`; see `--help`.
...
ROOM SOURCE / ASSETS:
    --rooms-dir <DIR>      room payload directory (default ~/palace-corpus/payloads_all)
    ...
    --roster <FILE>        .prp prop roster (default ~/palace-corpus/pserver.prp if present)
...
fn fill_defaults(args: &mut Args) {
    ...
    if args.media_roots.is_empty() {
        for candidate in [
            ...
            colosseum.join("arks"),
            PathBuf::from(&home).join("Pictures/Prop Files/Palace Media"),
            PathBuf::from(&home).join("Pictures/ThePalace Props/Media"),
            PathBuf::from(&home).join(".local/share/PalaceChat/Media"),
            PathBuf::from(&home).join("Sync/Colosseum-BGs/all"),
        ] {
            if candidate.is_dir() {
