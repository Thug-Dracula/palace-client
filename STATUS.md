...
**Scope, decisions and design:** `$CORPUS/TAURI-CLIENT-SCOPE.md` ← read that first, not this file's context.
...
Then clean up:
```bash
git worktree remove --force $REPO-asset
git worktree remove --force $REPO-render
...
`$REPO-render/renders/` — 8 real rooms composited from the corpus, each with a `.report.txt`:
...
- **Never read the research dumps** in `~/.local/share/agent-runner/tool-output/` — they are distilled into the scope doc. Reference file *paths*, never paste contents.
- **`$CORPUS/` is read-only reference.** Do not edit it. Beware Python tools creating `__pycache__` there.
