...
$CORPUS/tools/ and is never written to.

Usage:
    tools/diff_walker.py [--host localhost] [--port 9998]
                         [--user RustProbe] [--walker $CORPUS/tools/palace_walker.py]
...
DEFAULT_WALKER = os.path.expanduser("$CORPUS/tools/palace_walker.py")
...
def main():
    ...
    ap.add_argument(
        "--workspace",
        default=os.path.expanduser("$REPO"),
    )
    args = ap.parse_args()
