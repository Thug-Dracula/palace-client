...
2. decodes it with ``prop_decoder.py`` - the pre-existing, independently written
   8-bit decoder at ``$CORPUS/tools/prop_decoder.py`` - and compares again;
...
# `$CORPUS/` is a read-only reference. Loading `prop_decoder.py` from it would
# otherwise create or refresh `$CORPUS/tools/__pycache__/`, so bytecode writing
...
def main(argv: list[str]) -> int:
    ...
    prop_decoder = load_prop_decoder("$CORPUS/tools/prop_decoder.py")
